...
class GMM(object):
    ...
    def pdf(self,sigma,mu,x):
        return (1.0/np.sqrt(((2 * np.pi)**self.dimension/2)*np.linalg.det(sigma))) * np.exp(-0.5*np.transpose(x-mu)*np.linalg.inv(sigma)*(x-mu))
    ...
    def expectation(self):
        ...
        for i in range(self.SIZE):
            ...
            for j in range(self.K):
                ...
                self.p_clus[i][j] = self.p_clus[i][j]/np.sum(self.p_clus[i][j])
