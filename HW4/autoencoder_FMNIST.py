...
vectors = StandardScaler().fit_transform(FMNIST[1:-1, 1:])
X_train, X_test, y_train, y_test = train_test_split(vectors, labels, test_size=0.4, random_state=0)
...
for i in range(n):
    # MNIST test set
    batch_x, _ = X_test[i*n:i*n+n], y_test_hot[i*n:i*n+n]
